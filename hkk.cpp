...
//背景字符画
IMAGE homebackground, startgamebackground, makescorebackground, makescorebackground1, scoremodebackground, advancebackground, mchoosebackground, rchoosebackground, settingbackground, difficultybackground, volumbackground, speedbackground, rebornbackground, keypositionbackground;
IMAGE gamebackground1, gamebackground2, pausebackground1, pausebackground2, calculatebackground2, calculatebackground1, calculatebackground3, calculatebackground4, entertimebackground, enternamebackground, deletebackground, roadpositionbackground, recordbackground;
//小型素材
IMAGE kuangkuang, cursor, jiantou, yinfu, click, press, progressbar, progressbar0, longpress, precision[3], countnumber[3];
//字符集
IMAGE number[10], alpha[26], alpha1[26], alpha2[15], alpha3[7], alpha4[6], alpha5[4], rankalpha[6];
//动画
IMAGE animation[4];
struct aninode {
	aninode* next, *last;
...
void initial() {
	...
	SetConsoleTitle("黑色乐章");
	read(homebackground, "./picture/homebackground.txt");
	read(startgamebackground, "./picture/startgamebackground.txt");
	read(makescorebackground, "./picture/makescorebackground.txt");
	read(makescorebackground1, "./picture/makescorebackground1.txt");
	read(scoremodebackground, "./picture/scoremodebackground.txt");
	read(advancebackground, "./picture/advancebackground.txt");
	...
	read(gamebackground2, "./picture/gamebackground2.txt");
	read(pausebackground1, "./picture/pausebackground1.txt");
	read(pausebackground2, "./picture/pausebackground2.txt");
	read(calculatebackground1, "./picture/calculatebackground1.txt");
	read(calculatebackground2, "./picture/calculatebackground2.txt");
	read(calculatebackground3, "./picture/calculatebackground3.txt");
	read(calculatebackground4, "./picture/calculatebackground4.txt");
	read(entertimebackground, "./picture/entertimebackground.txt");
	read(enternamebackground, "./picture/enternamebackground.txt");
	read(deletebackground, "./picture/deletebackground.txt");
	read(settingbackground, "./picture/settingbackground.txt");
	read(difficultybackground, "./picture/difficultybackground.txt");
	read(volumbackground, "./picture/volumbackground.txt");
	read(speedbackground, "./picture/speedbackground.txt");
	read(rebornbackground, "./picture/rebornbackground.txt");
	read(keypositionbackground, "./picture/keypositionbackground.txt");
	read(roadpositionbackground, "./picture/roadpositionbackground.txt");
	read(recordbackground, "./picture/recordbackground.txt");
	...
	strcpy(name, "./precision/");
	for (int i = 0; i < 3; i++)
	{
		sprintf(tmp, "%d", i);
		strcat(name, tmp);
		strcat(name, ".txt");
		read(precision[i], name);
		strcpy(name, "./precision/");
	}
	strcpy(name, "./tool/");
	for (int i = 0; i < 5; i++)
	{
		...
		strcat(name, tmp);
		strcat(name, ".txt");
		read(rankalpha[i], name);
		strcpy(name, "./tool/");
	}
	read(rankalpha[5], "./tool/s.txt");
	strcpy(name, "./tool/count");
	for (int i = 0; i < 3; i++)
	{
		sprintf(tmp, "%d", i + 1);
		strcat(name, tmp);
		strcat(name, ".txt");
		read(countnumber[i], name);
		strcpy(name, "./tool/count");
	}
}
...
inline int ctik(const char* c, const cint xp) {//char to int with *1000
	...
	const cint c1 = strlen(c);
	int ans = cti(c) * 1000;
	for (cint x = xp + 1, w = 100; x < c1 && w && ans < 100000000; ++x, w /= 10)
		ans += (c[x] - '0') * w;
	return ans;
}
...
struct stdi {
	...
	bool inm = 0, ing = 0, ins = 0, inin, inen = 0, inch = 0;
	int totn;
	int bgv, efv;//bgm音量 特效音量
	int sks, skd;//添加记录点键位 删除记录点键位
	cint inmo;//0名字 1数字 2时间 3键位
	...
	struct stpi {
		...
		int ntp[K][B], ntk[K][B], nt1[K], sk[K];//音符的位置(note position) 音符的类型 一条线上音符的数量
		bool nk[4];//在第几条道上在按
		double spe;
	...
	void inianimation(cint mode) {
		for (int i = 0; i < 4; i++)
		{
			aninode* pn = p1.head[i].next;
			while (pn != NULL && pn->ind != -1) {
				aninode* tmp = pn->next;
	...
	inline void bger() {//歌曲导入失败(bgm error)----------------------------------------------------------------------
		MessageBox(NULL, "歌曲导入失败!", "会不会玩？", MB_OK);
		for (; pdk(enter1) || pdk(enter2););
	}
	inline void puer() {//谱面导入失败(pu error)----------------------------------------------------------------------
		MessageBox(NULL, "谱面导入失败!", "会不会玩？", MB_OK);
		for (; pdk(enter1) || pdk(enter2););
	}
	inline void nuer() {//数字有问题(number error)----------------------------------------------------------------------
		MessageBox(NULL, "确定宁输入的数字没有问题?!", "会不会玩？", MB_OK);
		for (; pdk(enter1) || pdk(enter2););
	}
	inline void ser() {//字符串有问题(number error)----------------------------------------------------------------------
		MessageBox(NULL, "确定宁输入的名称没有问题?!", "会不会玩？", MB_OK);
		for (; pdk(enter1) || pdk(enter2););
	}
		...
		*/
		if (ing) {
			if (mode != 2) {
				...
				//打击精度绘制
				if (p1.pgb == 0 || p1.pgb == 1 || p1.pgb == 2)
					paint(2, 68, precision[p1.pgb], buffer);
				...
				for (int k = 0; k < 4; k++)
				{
					...
					{
						if (p1.ntk[k][i] == 1) {
							paint(k * 63 + 64, HIGH - 4 - p1.ntp[k][i], click, buffer);
						}
						else if (p1.ntk[k][i] == 2)
						{
							paintud(k * 63 + 64, 0, longpress, HIGH - 1 - p1.ntp[k][i + 1], HIGH - 3 - p1.ntp[k][i], buffer);
							paint(k * 63 + 64, HIGH - 7 - p1.ntp[k][i], press, buffer);
							++i;
						}
						else if (p1.ntk[k][i] == 3)
						{
							paintud(k * 63 + 64, 0, longpress, HIGH - 1 - p1.ntp[k][i], 211, buffer);
						}
					}
				}
				for (int k = 0; k < 4; k++)
				{
					for (int i = 0; i < p2.nt1[k]; i++)
					{
						if (p2.ntk[k][i] == 1) {
							paint(k * 63 + 308, HIGH - 4 - p2.ntp[k][i], click, buffer);
						}
						else if (p2.ntk[k][i] == 2)
						{
							paintud(k * 63 + 308, 0, longpress, HIGH - 1 - p2.ntp[k][i + 1], HIGH - 3 - p2.ntp[k][i], buffer);
							paint(k * 63 + 308, HIGH - 7 - p2.ntp[k][i], press, buffer);
							++i;
						}
						else if (p2.ntk[k][i] == 3)
						{
							paintud(k * 63 + 308, 0, longpress, HIGH - 1 - p2.ntp[k][i], 211, buffer);
						}
					}
				}
				//进度条及背景绘制
				paint(0, 0, gamebackground2, buffer);
				int popos = int(double(nt) / double(tlen) * 497) + 62;
				paint(0, 0, progressbar0, buffer);
				paintlr(0, 0, progressbar, 0, popos, buffer);
				//动画序列插入
				...
				for (int k = 0; k < 4; k++)
				{
					if (p2.nk[k]) {
						aninode* pn = new aninode;
						pn->last = p2.end[k].last; pn->next = &p2.end[k];
						p2.end[k].last->next = pn;
						p2.end[k].last = pn;
						pn->ind = 0;
					}
				}
				...
				if (!inm) {
					for (int k = 0; k < 4; k++)
					{
						aninode* pn = p1.head[k].next;
						while (pn->ind != -1) {
							paint(k * 63 + 65, 194, animation[pn->ind], buffer);
							++(pn->ind);
							aninode* tmp = pn->next;
							if (pn->ind == 5) {
								pn->last->next = pn->next;
					...
					for (int k = 0; k < 4; k++)
					{
						aninode* pn = p2.head[k].next;
						while (pn->ind != -1) {
							paint(k * 63 + 309, 194, animation[pn->ind], buffer);
							++(pn->ind);
							aninode* tmp = pn->next;
							if (pn->ind == 5) {
								pn->last->next = pn->next;
				...
				if (p1.pgb == 0 || p1.pgb == 1 || p1.pgb == 2)
					paint(2, 68, precision[p1.pgb], buffer);
				if (p2.pgb == 0 || p2.pgb == 1 || p2.pgb == 2)
					paint(555, 68, precision[p1.pgb], buffer);
			}
			if (!inm)
				strcpy(tmpbackground, buffer);
			else {
				strcpy(buffer, tmpbackground);
				paint(0, 0, pausebackground1, buffer);
				paint(238, 57 + 35 * sx, kuangkuang, buffer);
				if (menu == 52) {
					int i = 308, j = 63 + sx * 35;
					paint(0, 0, pausebackground2, buffer);
					if (inin) {
						...
						paint(sex * 12 + 305, j - 3, cursor, buffer);
					}
					else {
						string s = nts(bgv);
						int i = 308, j = 63;
			...
			if (inch) {
				if (nx != K)
					paint(nx * 63 + 191, HIGH - p1.ntp[nx][np] - 15, yinfu, buffer);
			}
			...
			settoblank(buffer);
			if (mode == 0) {
				paint(0, 0, calculatebackground1, buffer);
				string s = nts(hs);
				int i = 167, j = 25;
				for (int k = 0; k < s.length(); k++)
				{
				...
				if (p1.rank == 'S')paint(139, 44, rankalpha[5], buffer);
				else paint(139, 44, rankalpha[p1.rank - 'A'], buffer);
				s = nts(p1.sc);
				i = 139, j = 76;
				for (int k = 0; k < s.length(); k++)
				{
				...
				paint(sx * 309 + 61, 197, kuangkuang, buffer);
			}
			if (mode == 1) {
				paint(0, 0, calculatebackground4, buffer);
				string s = nts(p1.sc);
				int i = 139, j = 76;
				for (int k = 0; k < s.length(); k++)
				{
			...
			if (mode == 2) {
				paint(0, 0, calculatebackground2, buffer);
				if (p1.rank == 'S')paint(139, 44, rankalpha[5], buffer);
				else paint(139, 44, rankalpha[p1.rank - 'A'], buffer);
				if (p2.rank == 'S')paint(448, 44, rankalpha[5], buffer);
				else paint(448, 44, rankalpha[p2.rank - 'A'], buffer);
				string s = nts(p1.sc);
				int i = 139, j = 76;
				for (int k = 0; k < s.length(); k++)
				{
			...
			if (mode == 3) {
				paint(0, 0, calculatebackground3, buffer);
				paint(sx * 309 + 61, 197, kuangkuang, buffer);
			}
			render(buffer, hout, sout);
			buffertoview(hout, sout);
			...
			pair<int, int> pos[5] = { {73,70},{73,106},{73,141},{73,176} ,{73,211} };
			if (inin != 1) {
				if (menu == 1) {
					settoblank(buffer);
					...
					settoblank(buffer);
					paint(0, 0, makescorebackground, buffer);
					paint(pos[sx].first, pos[sx].second, kuangkuang, buffer);
					render(buffer, hout, sout);
					buffertoview(hout, sout);
				}
					...
					settoblank(buffer);
					paint(0, 0, makescorebackground1, buffer);
					paint(pos[sx].first, pos[sx].second, kuangkuang, buffer);
					render(buffer, hout, sout);
					buffertoview(hout, sout);
				}
					...
					settoblank(buffer);
					paint(0, 0, scoremodebackground, buffer);
					paint(pos[sx].first, pos[sx].second, kuangkuang, buffer);
					render(buffer, hout, sout);
					buffertoview(hout, sout);
				}
					...
					settoblank(buffer);
					paint(0, 0, advancebackground, buffer);
					paint(pos[sx].first, pos[sx].second, kuangkuang, buffer);
					render(buffer, hout, sout);
					buffertoview(hout, sout);
				}
					...
					settoblank(buffer);
					paint(0, 0, deletebackground, buffer);
					paint(sx * 234 + 128, 163, kuangkuang, buffer);
					render(buffer, hout, sout);
					buffertoview(hout, sout);
				}
				else if (menu == 4) {
					settoblank(buffer);
					paint(0, 0, settingbackground, buffer);
					paint(pos[sx].first, pos[sx].second, kuangkuang, buffer);
					render(buffer, hout, sout);
					buffertoview(hout, sout);
				}
					...
					settoblank(buffer);
					paint(0, 0, difficultybackground, buffer);
					paint(pos[sx].first, pos[sx].second, kuangkuang, buffer);
					render(buffer, hout, sout);
					buffertoview(hout, sout);
				}
					...
					settoblank(buffer);
					paint(0, 0, volumbackground, buffer);
					paint(pos[sx].first, pos[sx].second, kuangkuang, buffer);
					string s = nts(bgv);
					int i = 143, j = 76;
					for (int k = 0; k < s.length(); k++)
					...
					settoblank(buffer);
					paint(0, 0, speedbackground, buffer);
					paint(pos[sx].first, pos[sx].second, kuangkuang, buffer);
					string s = nts(p1.spe);
					int i = 143, j = 76;
					for (int k = 0; k < s.length(); k++)
					...
					settoblank(buffer);
					paint(0, 0, keypositionbackground, buffer);
					paint(pos[sx].first, pos[sx].second, kuangkuang, buffer);
					render(buffer, hout, sout);
					buffertoview(hout, sout);
				}
					...
					settoblank(buffer);
					paint(0, 0, roadpositionbackground, buffer);
					paint(pos[sx].first, pos[sx].second, kuangkuang, buffer);
					int i, j;
					for (int k = 0; k < 4; k++)
					{
						char c = info.pdp(p1.sk[k]);
						if (c == 0)c = p1.sk[k];
						i = pos[k].first + 75, j = pos[k].second + 6;
						if (c >= '0' && c <= '9') {
							paint(i, j, number[c - '0'], buffer);
						}
						else if (c >= 'a' && c <= 'z') {
							paint(i, j, alpha[c - 'a'], buffer);
						}
						else if (c >= 'A' && c <= 'Z') {
							paint(i, j, alpha1[c - 'A'], buffer);
						}
						else if (c >= 33 && c <= 47) {
							paint(i, j, alpha2[c - 33], buffer);
						}
						else if (c >= 58 && c <= 64) {
							paint(i, j, alpha3[c - 58], buffer);
						}
						else if (c >= 91 && c <= 96) {
							paint(i, j, alpha4[c - 33], buffer);
						}
						else if (c >= 123 && c <= 126) {
							paint(i, j, alpha1[c - 123], buffer);
						}
					}
					render(buffer, hout, sout);
					buffertoview(hout, sout);
					...
					settoblank(buffer);
					paint(0, 0, recordbackground, buffer);
					paint(pos[sx].first, pos[sx].second, kuangkuang, buffer);
					int i, j;
					for (int k = 0; k < 2; k++)
					{
						char tmp = k == 0 ? sks : skd;
						char c = info.pdp(tmp);
						if (c == 0)c = tmp;
						i = pos[k].first + 75, j = pos[k].second + 6;
						if (c >= '0' && c <= '9') {
				...
				settoblank(buffer);
				paint(0, 0, entertimebackground, buffer);
				int i = 73, j = 40;
				for (int k = 0; k < ses.length(); k++)
				{
					if (ses[k] >= '0' && ses[k] <= '9') {
				...
				paint(sex * 16 + 67, 37, cursor, buffer);
				render(buffer, hout, sout);
				buffertoview(hout, sout);
			}
			else if (inmo == 0) {
				settoblank(buffer);
				paint(0, 0, enternamebackground, buffer);
				int i = 73, j = 40;
				for (int k = 0; k < ses.length(); k++)
				{
					if (ses[k] >= '0' && ses[k] <= '9') {
				...
				if (menu == 52) {
					...
					paint(sex * 12 + 140, j - 3, cursor, buffer);
					render(buffer, hout, sout);
					buffertoview(hout, sout);
				}
				else if (menu == 53) {
					...
					settoblank(buffer);
					paint(0, 0, rebornbackground, buffer);
					paint(73, 70, kuangkuang, buffer);
					int i = 143, j = 76;
					for (int k = 0; k < ses.length(); k++)
					{
						if (ses[k] >= '0' && ses[k] <= '9') {
				...
				paint(pos[sx].first, pos[sx].second, kuangkuang, buffer);
				int i, j;
				i = pos[sx].first + 75, j = pos[sx].second + 6;
				for (int k = 0; k < ses.length(); k++)
				{
					if (ses[k] >= '0' && ses[k] <= '9') {
						paint(i, j, number[ses[k] - '0'], buffer);
				...
				paint(sex * 12 + 145, j - 3, cursor, buffer);
				render(buffer, hout, sout);
				buffertoview(hout, sout);
	...
	inline void c321(double num) {//倒数321(cout 3 2 1)----------------------------------------------------------------------
		static int ds, lac;
		ds = num * 1000;
		const long long lat = clock();
		long long noti;
		strcpy(buffer, tmpbackground);
		paint(0, 0, countnumber[(lac = ceil((double)ds / 1000)) - 1], buffer);
		render(buffer, hout, sout);
		buffertoview(hout, sout);
		settoblank(buffer);
		if (mode == 2)paint(0, 0, gamebackground2, buffer);
		else paint(0, 0, gamebackground1, buffer);
		for (--lac; (noti = ds - (clock() - lat)) > 0;)
			if (noti / 1000 < lac) {
				strcpy(buffer, tmpbackground);
				paint(0, 0, countnumber[(lac--) - 1], buffer);
				render(buffer, hout, sout);
				buffertoview(hout, sout);
				settoblank(buffer);
				if (mode == 2)paint(0, 0, gamebackground2, buffer);
...
}bgm, ml, dea, begi, clic, ente;//bgm menuloop finish death
...
struct stme {
	...
	inline void disp() {//显示(display)
		...
		info.sks = rp.sks;
		info.skd = rp.skd;
		info.p1.spe = p1.spe / 100;
		info.p2.spe = p2.spe / 100;
		for (cint x = 0; x < K; ++x) {
			info.p1.sk[x] = p1.sk[x];
			info.p2.sk[x] = p2.sk[x];
		}
		info.menu = st[p].p;
		if (pdch(st[p].p)) {
			info.sex = sex;
	...
	inline void mian() {//mian函数
		...
		for (; p > 0;) {
			...
			else if (!(st[p].p == ens || st[p].p == endi) && (nk == up || nk == down) || (st[p].p == ens || st[p].p == endi) && (nk == left || nk == right)) {
				static cint k;
				if (nk == up || nk == left)
					k = -1;
				else
			...
			if (st[p].p <= 0) {
				...
				else if (st[p].p == p1s && (tems = ips.ges("P1音符下落速度（最小1，最大10）： ", 1, nts(p1.spe / 100))).size())
					p1.spe = max((double)100, min((double)1000, ctd(tems.c_str()) * 100));
				else if (st[p].p == p2s && (tems = ips.ges("P2音符下落速度（最小1，最大10）： ", 1, nts(p2.spe / 100))).size())
					p2.spe = max((double)100, min((double)1000, ctd(tems.c_str()) * 100));
				else if (st[p].p == rbt && (tems = ips.ges("练习模式重生时间（最小0，最大5）： ", 1, nts(gs.cnt))).size())
					gs.cnt = min((double)3, ctd(tems.c_str()));
...
struct stpl {
	inline void pdnt(stpi& p, const cint x) {//判断音符(pd note)
		...
		if (p.k[x] && !p.lak[x] && npt <= nt + gs.pb) {//区域内按
			...
			if (p.lak[x] && npt <= nt + gs.pg && !p.k[x] && p.lak[x] || p.lak[x] && npt <= nt && p.k[x]) {
				if (p.lk[x] == 2)
					p.adpf();
				else
					p.adgd();
	...
	inline void cpp(stdi::stpi& cp, stpi& p, const cint mode) {//复制玩家信息(copy player)
		cp.sc = p.sc;
		cp.pf = p.pf;
		cp.gd = p.gd;
		cp.ba = p.ba;
	...
	inline bool puen(const cint mode) {//判断谱面是否完成(pu end)
		return p1.puen() && (mode != 2 || p2.puen()) || mode == 3;
	}
	...
	inline void pl(const int bgid, const int puid, const cint mode, const int st, const cint crm) {//单人游戏:0表示读曲失败 st:开始时间(start) mode:0是单人模式，1是练习模式，2是双人模式，3是制谱模式 crm:0是自动模式，1是短按，2是长按
		...
		for (++bj, p1.ex = 1; p1.ex || p1.al; ++bj) {
			...
			info.ing = 1;
			p1.ini(nt = rp.sta[rp.st1]);
			info.inid();
			info.p1.pgb = info.p2.pgb = p1.pgb;
			disp(mode);
			info.c321(c321(mode));
